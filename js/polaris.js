...
function showPolarisTab(name){
  ['roster','assignments','lessons','guides'].forEach(n=>{
    const el=document.getElementById('prg-sub-'+n);if(el)el.style.display=n===name?'':'none';
    const btn=document.getElementById('ptab-'+n);if(btn)btn.classList.toggle('active',n===name);
  });
}

// ── POLARIS: ASSIGNMENTS ──────────────────────────────────────────────────────
function addPolarisAssignment(){
  ['pa-title','pa-assignedto','pa-grade','pa-notes'].forEach(id=>document.getElementById(id).value='');
...
function submitPolarisAssignment(){
  const title=document.getElementById('pa-title').value.trim();if(!title)return;
...
function renderPolarisAssignments(){
  const t=document.getElementById('polaris-assignments-table');if(!t)return;
  ...
  const items=_polarisAssignments.filter(a=>(!sf||a.status===sf)&&(!cf||a.category===cf));
  if(!items.length){t.innerHTML='<tr><td colspan="7" style="font-size:11px;color:var(--t4);padding:12px">No assignments yet. Click "+ Add Assignment" to get started.</td></tr>';return;}
  t.innerHTML=items.map(a=>{
...
function deletePolarisAssignment(i){_polarisAssignments.splice(i,1);saveRoleData('polarisassignments',_polarisAssignments);renderPolarisAssignments();}

// ── POLARIS: LESSON PLANS ─────────────────────────────────────────────────────
function addPolarisLesson(){
  ['pl-title','pl-objective','pl-materials','pl-agenda','pl-assessment','pl-notes-lesson'].forEach(id=>document.getElementById(id).value='');
...
function submitPolarisLesson(){
  const title=document.getElementById('pl-title').value.trim();if(!title)return;
  ...
  saveRoleData('polarislessons',_polarisLessons);renderPolarisLessons();closeModals();
}
function renderPolarisLessons(){
  const el=document.getElementById('polaris-lessons-list');if(!el)return;
  if(!_polarisLessons.length){el.innerHTML='<div style="padding:24px;text-align:center;color:var(--t4);font-size:11px">No lesson plans yet. Click "+ New Lesson Plan" to create one.</div>';return;}
  el.innerHTML=_polarisLessons.map((l,i)=>`
...
function previewLessonPlan(i){
  ...
  const fmt=s=>s?(s.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/\n/g,'<br>')):'—';
  const sec=(h,b)=>b&&b!=='—'?`<div style="margin-bottom:14px"><div style="font-size:10px;font-weight:700;color:var(--t3);text-transform:uppercase;letter-spacing:.05em;margin-bottom:4px">${h}</div><div style="font-size:12px;color:var(--t1);line-height:1.7">${b}</div></div>`:'';
...
function printLessonPlan(){
  ...
  w.document.write(`<!DOCTYPE html><html><head><title>${title}</title><style>body{font-family:'Segoe UI',sans-serif;padding:36px 48px;color:#111;max-width:640px;margin:0 auto}h1{font-size:18px;font-weight:700;margin-bottom:20px;border-bottom:2px solid #111;padding-bottom:8px}@media print{body{padding:20px}}</style></head><body><h1>${title}</h1>${body}</body></html>`);
  w.document.close();w.focus();setTimeout(()=>w.print(),350);
}
function deletePolarisLesson(i){_polarisLessons.splice(i,1);saveRoleData('polarislessons',_polarisLessons);renderPolarisLessons();}
...
function submitPolarisGuide(){
  const title=document.getElementById('pg-title').value.trim();if(!title)return;
  _polarisGuides.push({title,category:document.getElementById('pg-category').value,content:document.getElementById('pg-content').value.trim(),icon:'📄',pinned:false,open:false});
  saveRoleData('polarisguides',_polarisGuides);renderPolarisGuides();closeModals();
}
function renderPolarisGuides(){
  const el=document.getElementById('polaris-guides-list');if(!el)return;
  const q=((document.getElementById('pg-search')||{}).value||'').toLowerCase();
  const catOrder=['Writing','Presentation','Research','Strategy','Other'];
  const items=_polarisGuides.filter((g,i)=>{g._i=i;return!q||(g.title+g.content).toLowerCase().includes(q);});
  if(!items.length){el.innerHTML='<div style="padding:16px;text-align:center;color:var(--t4);font-size:11px">No guides found.</div>';return;}
  const grouped={};
  items.forEach(g=>{const c=g.category||'Other';if(!grouped[c])grouped[c]=[];grouped[c].push(g);});
...
function togglePolarisGuide(i){if(_polarisGuides[i])_polarisGuides[i].open=!_polarisGuides[i].open;renderPolarisGuides();}
function deletePolarisGuide(i){if(!_polarisGuides[i]||_polarisGuides[i].pinned)return;_polarisGuides.splice(i,1);saveRoleData('polarisguides',_polarisGuides);renderPolarisGuides();}
